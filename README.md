...
│
└─ src
   │  index.ts  // the main entry-point to the application for Razzle
   └─ client
   │  │  index.tsx   // entry point to the client
   │  │  App.tsx     // the main component (used in both SSR and CSR)
   ...
   │           │  use[HookName].ts  // a custom hook used specifically on this screen
   │
   └─ server
      │  index.tsx  // entry point to the server
      └─ api        // the routes of the api
      │  |
      │  │
      │  │
      │  └─ index.ts  // entry point for the routes  api routes
      │  │
      │  │
